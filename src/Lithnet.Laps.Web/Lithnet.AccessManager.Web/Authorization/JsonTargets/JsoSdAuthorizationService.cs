...
{
    public class JsoSdAuthorizationService : IAuthorizationService
    {
        ...
        public JsoSdAuthorizationService(IDirectory directory, ILogger logger, IJsonTargetsProvider provider, IAceEvaluator aceEvaluator, IJitAccessGroupResolver jitResolver)
        {
        ...
        public AuthorizationResponse GetAuthorizationResponse(IUser user, IComputer computer, AccessMask requestedAccess)
        {
            ...
            List<IJsonTarget> successfulTargets = new List<IJsonTarget>();
            List<IJsonTarget> failedTargets = new List<IJsonTarget>();
            ...
            foreach (var target in targets)
            {
                ...
                {
                    target.NotificationChannels?.OnFailure?.ForEach(u => failureNotificationRecipients.Add(u));
                }
            }
            ...
            else
            {
                ...
                this.logger.Trace($"User {user.MsDsPrincipalName} is authorized to read passwords for computer {computer.MsDsPrincipalName} from target {j.Name}");

                return BuildAuthZResponseSuccess(requestedAccess, j, null, computer);
            }
        }
